...
function SectionEyebrow({ children }: { index?: string; children: ReactNode }) {
  return <div className="section-eyebrow"><i /><p>{children}</p></div>;
}
  ...
  return (
    <main dir="rtl" className="site-shell">
      <header className="site-header">
        ...
        <div className="header-actions"><a href="tel:+967780500363" className="phone-link"><span dir="ltr">+967 780 500 363</span></a><button className="outline-button header-cta" onClick={openBooking}>احجز موعدًا <ArrowIcon /></button><button className="menu-toggle" aria-label={mobileOpen ? "إغلاق القائمة" : "فتح القائمة"} aria-expanded={mobileOpen} onClick={() => setMobileOpen((value) => !value)}><span /><span /></button></div>
      </header>

      <section id="top" className="hero-section">
        <div className="hero-art" aria-hidden="true" onPointerMove={handleHeroPointerMove} onPointerLeave={resetHeroPointer}><Image src="/hero-porcelain-left.webp" alt="" fill priority sizes="(max-width: 900px) 100vw, 54vw" /><div className="hero-art-wash" /><div className="hero-art-label">PRECISION</div></div>
      ...
      <section className="principles-section section-space"><div className="page-width"><SectionEyebrow index={offers.length ? "05" : "04"}>لماذا د. ليان</SectionEyebrow><div className="principles-intro"><h2>لا نعدك<br /><em>بالمزيد.</em><br />نعدك بالأفضل.</h2><p>كل قرار علاجي لدينا يمر عبر أربعة مبادئ بسيطة: الدقة، الوضوح، الراحة، والنتيجة الطبيعية.</p></div><div className="principles-list"><div><strong>دقة تشخيصية</strong><p>نرى الصورة كاملة قبل أن نبدأ.</p></div><div><strong>خطة مفهومة</strong><p>تعرف ماذا سنفعل ولماذا.</p></div><div><strong>راحة مستمرة</strong><p>تجربة هادئة في كل زيارة.</p></div><div><strong>نتيجة طبيعية</strong><p>ابتسامة تشبهك، لا نسخة من أحد.</p></div></div></div></section>
