...
function Services({ data, reload, showNotice }: { data: DashboardData; reload: () => void; showNotice: (message: string) => void }) {
  ...
  const save = async (event: FormEvent) => { event.preventDefault(); try { await requestJson(editing ? `/api/admin/services/${editing}` : "/api/admin/services", { method: editing ? "PATCH" : "POST", body: JSON.stringify({ ...form, sort_order: Number(form.sort_order) }) }); showNotice(editing ? "تم تحديث الخدمة." : "تمت إضافة الخدمة."); setEditing(null); setForm(emptyService); reload(); } catch (error) { showNotice(error instanceof Error ? error.message : "تعذر الحفظ."); } };
  ...
  return <div className="admin-content"><div className="content-toolbar"><div><span className="panel-kicker">محتوى قابل للتحكم</span><p>الخدمات التي تظهر في صفحة الموقع العامة.</p></div><button className="admin-primary-button compact" onClick={() => { setEditing(null); setForm(emptyService); }}>+ إضافة خدمة</button></div><div className="admin-editor-layout"><section className="admin-panel-card editor-list">{data.services.map((service) => <div className={`editor-row ${!service.active ? "is-muted" : ""}`} key={service.id}><img src={service.image_url} alt="" /><div><strong>{service.title}</strong><small>{service.english}</small><em>{service.active ? "نشطة على الموقع" : "مخفية عن الموقع"}</em></div><div className="row-actions"><button onClick={() => edit(service)}>تعديل</button><button className="danger-text" onClick={() => remove(service.id)}>حذف</button></div></div>)}</section><EditorForm title={editing ? "تعديل الخدمة" : "خدمة جديدة"} onSubmit={save} onCancel={() => { setEditing(null); setForm(emptyService); }}><Field label="اسم الخدمة" value={form.title} onChange={(value) => setForm({ ...form, title: value })} /><Field label="الاسم الإنجليزي" value={form.english} onChange={(value) => setForm({ ...form, english: value })} /><Field label="الوصف" value={form.description} multiline onChange={(value) => setForm({ ...form, description: value })} /><Field label="رابط الصورة" value={form.image_url} onChange={(value) => setForm({ ...form, image_url: value })} /><div className="form-grid"><Field label="الترتيب" value={String(form.sort_order)} type="number" onChange={(value) => setForm({ ...form, sort_order: Number(value) })} /><label className="toggle-field"><input type="checkbox" checked={form.active} onChange={(event) => setForm({ ...form, active: event.target.checked })} /><span>تظهر على الموقع</span></label></div></EditorForm></div></div>;
}
...
function Offers({ data, reload, showNotice }: { data: DashboardData; reload: () => void; showNotice: (message: string) => void }) {
  ...
  const save = async (event: FormEvent) => { event.preventDefault(); try { await requestJson(editing ? `/api/admin/offers/${editing}` : "/api/admin/offers", { method: editing ? "PATCH" : "POST", body: JSON.stringify({ ...form, discount_percent: Number(form.discount_percent), sort_order: Number(form.sort_order), valid_until: form.valid_until || "" }) }); showNotice(editing ? "تم تحديث العرض." : "تمت إضافة العرض."); setEditing(null); setForm(emptyOffer); reload(); } catch (error) { showNotice(error instanceof Error ? error.message : "تعذر الحفظ."); } };
...
function Media({ data, reload, showNotice }: { data: DashboardData; reload: () => void; showNotice: (message: string) => void }) {
  ...
  const save = async (event: FormEvent) => { event.preventDefault(); try { await requestJson(editing ? `/api/admin/media/${editing}` : "/api/admin/media", { method: editing ? "PATCH" : "POST", body: JSON.stringify({ ...form, sort_order: Number(form.sort_order) }) }); showNotice(editing ? "تم تحديث الصورة." : "تمت إضافة الصورة."); setEditing(null); setForm(emptyMedia); reload(); } catch (error) { showNotice(error instanceof Error ? error.message : "تعذر الحفظ."); } };
