...
type Review = { id: number; author_name: string; content: string; service_name: string; rating: number; sort_order: number; active: boolean };
type DashboardData = { bookings: Booking[]; services: Service[]; offers: Offer[]; media: MediaItem[]; reviews: Review[]; stats: { total: string; new_count: string; confirmed_count: string; today_count: string } };
...
const emptyReview = { author_name: "", content: "", service_name: "", rating: 5, sort_order: 1, active: true };
...
function Reviews({ data, reload, showNotice }: { data: DashboardData; reload: () => void; showNotice: (message: string) => void }) {
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      await requestJson(editing ? `/api/admin/reviews/${editing}` : "/api/admin/reviews", { method: editing ? "PATCH" : "POST", body: JSON.stringify({ ...form, rating: Number(form.rating), sort_order: Number(form.sort_order) }) });
  ...
  const edit = (review: Review) => { setEditing(review.id); setForm({ author_name: review.author_name, content: review.content, service_name: review.service_name, rating: review.rating, sort_order: review.sort_order, active: review.active }); };
  ...
  return <div className="admin-content"><div className="content-toolbar"><div><span className="panel-kicker">محتوى حقيقي من Neon</span><p>أضف تجارب العملاء بعد موافقتهم، ثم تحكم في ظهورها على الموقع.</p></div><button className="admin-primary-button compact" onClick={() => { setEditing(null); setForm(emptyReview); }}>+ إضافة مراجعة</button></div><div className="admin-editor-layout"><section className="admin-panel-card review-admin-list">{data.reviews.length === 0 ? <EmptyState label="لا توجد مراجعات بعد. أضف أول تجربة عميل." /> : data.reviews.map((review) => <article className={`review-admin-row ${!review.active ? "is-muted" : ""}`} key={review.id}><div className="review-admin-index"><span>{String(review.sort_order).padStart(2, "0")}</span><b>{review.rating}/5</b></div><div className="review-admin-copy"><strong>{review.author_name}</strong><small>{review.service_name}</small><p>{review.content}</p><em>{review.active ? "ظاهرة على الموقع" : "مخفية عن الموقع"}</em></div><div className="row-actions"><button onClick={() => edit(review)}>تعديل</button><button className="danger-text" onClick={() => remove(review.id)}>حذف</button></div></article>)}</section><EditorForm title={editing ? "تعديل المراجعة" : "مراجعة جديدة"} onSubmit={save} onCancel={() => { setEditing(null); setForm(emptyReview); }}><Field label="اسم العميل" value={form.author_name} onChange={(value) => setForm({ ...form, author_name: value })} placeholder="مثال: سارة م." /><Field label="الخدمة" value={form.service_name} onChange={(value) => setForm({ ...form, service_name: value })} placeholder="مثال: تصميم ابتسامة" /><Field label="نص المراجعة" value={form.content} multiline onChange={(value) => setForm({ ...form, content: value })} placeholder="اكتب تجربة العميل بعد الحصول على موافقته." /><div className="form-grid"><Field label="التقييم من 1 إلى 5" value={String(form.rating)} type="number" onChange={(value) => setForm({ ...form, rating: Number(value) })} /><Field label="ترتيب العرض" value={String(form.sort_order)} type="number" onChange={(value) => setForm({ ...form, sort_order: Number(value) })} /></div><label className="toggle-field"><input type="checkbox" checked={form.active} onChange={(event) => setForm({ ...form, active: event.target.checked })} /><span>تظهر على الموقع</span></label><p className="editor-help">لن تظهر المراجعة للزوار إلا عندما تكون مفعلة. يمكنك إخفاؤها مؤقتًا دون حذفها من قاعدة البيانات.</p></EditorForm></div></div>;
}
